...
import { formatDuration, formatDigitalTime } from '../../utils/formatters'; 

const SessionHistoryList = ({ history, onImageClick }) => {
    
    const groupedData = history.reduce((acc, session) => {
        const dateObj = new Date(session.end_time || session.created_at);
                            ...
                            return (
                                <div 
                                    ...
                                    className={`
                                        ...
                                        ${hasPhoto ? 'grid-cols-[65px_1fr_auto]' : 'grid-cols-[65px_1fr]'}
                                    `}
                                >
                                    <div className="text-muted/60 font-bold text-[11px] pt-0.5 group-hover:text-blood transition-colors">
                                        {formatDigitalTime(session.duration_seconds)}
                                    </div>
                                    
                                    <div className="text-xs text-muted group-hover:text-bone whitespace-pre-wrap leading-relaxed wrap-break-word min-w-0 transition-colors">
