...
const CollectionDetailsPage = () => {
    ...
    const handleSaveToggle = async () => {
        ...
        try {
            if (isSaved) {
                ...
                setIsSaved(false);
                setCollection(prev => ({ ...prev, save_count: Math.max(0, (prev.save_count || 0) - 1) }));
            } else {
                await collectionService.saveCollection(id);
                setIsSaved(true);
                setCollection(prev => ({ ...prev, save_count: (prev.save_count || 0) + 1 }));
            }
        } catch (error) {
    ...
    // 👇 ЦЯ ОБГОРТКА ВИРІШУЄ ПРОБЛЕМУ КЛІКУ
    const ArtWrapper = ({ artwork, children, className }) => {
        if (isOwner) {
            // Власник йде на сторінку редагування/деталей
                ...
                <div className="absolute top-8 right-4 md:right-8 flex items-center gap-4">
                    {!isOwner && (
                        <button 
                            ...
                            disabled={isSaving}
                            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition border ${isSaved ? 'bg-cherry-900/20 text-cherry-400 border-cherry-900/50 hover:bg-cherry-900/30' : 'bg-slate-900 text-slate-400 border-slate-700 hover:text-white hover:border-slate-500'}`}
                        >
                ...
                <div className="max-w-7xl mx-auto space-y-32 px-4 py-10">
                    {collection.items.map((art) => {
                        ...
                        const imageWrapperClass = art.layout_type === 'CENTER' 
                            ...
                            : 'w-full md:w-1/2 h-[500px] md:h-[600px]';
                        ...
                        return (
                            <div key={art.link_id} className={layoutClasses}>
                                <div className={`relative shadow-2xl rounded-sm overflow-hidden border-[8px] border-white bg-black ${imageWrapperClass}`}>
                                    {/* 👇 ВИПРАВЛЕНО: Тепер тут теж ArtWrapper */}
                                    <ArtWrapper artwork={art} className="block w-full h-full">
                                        <AtmosphereImage 
                                            src={artworkService.getImageUrl(art.image_path)} 
                                            alt={art.title} 
